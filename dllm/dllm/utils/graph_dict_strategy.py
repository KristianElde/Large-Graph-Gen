...
def extract_graph_dict_text(text: str) -> str:
    ...
    The parser only understands the dictionary part. This helper returns the
    substring beginning at the first "{" and, when possible, ending at the
    matching closing "}".
    """
    if not text:
        return ""
    ...
    # No matching final brace. Return the candidate so parsing can still try,
    # but strict validity should mark it invalid.
