...
def forward_qwen2(
    ...
    """Patched Qwen2Model.forward that accepts 4D attention masks (bidirectional).

    Mirrors forward_llama2 above. The only structural difference from Llama is
    that Qwen2 doesn't pre-compute position_embeddings at the model level — each
    decoder layer computes its own rotary embeddings internally.
    """
    output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
    output_hidden_states = (
        output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
    ...
    if use_cache and not isinstance(past_key_values, Cache) and not self.training:
        ...
        past_key_values = DynamicCache.from_legacy_cache(past_key_values)
        logger.warning_once(
            "We detected that you are passing `past_key_values` as a tuple. This is deprecated."
        )
    ...
    ## Add by Qwen2-diffusion port, adapting for 4d attention-mask.
    if attention_mask is not None and len(attention_mask.shape) == 4:
        causal_mask = attention_mask
        print("logging....attention-mask for 4d (qwen2)")
    else:
        causal_mask = self._update_causal_mask(
            attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions
        )
...
def replace_attention_mask():
    ...
    # Qwen2 diffusion port
    try:
        import transformers.models.qwen2.modeling_qwen2 as _qwen2_mod
        _qwen2_mod.Qwen2Model.forward = forward_qwen2
        print("  + Qwen2Model.forward patched for bidirectional attention")
    except (ImportError, AttributeError) as e:
        print(f"  - Qwen2 not patched ({e})")
