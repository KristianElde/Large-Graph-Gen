...
class DiscreteDiffusionModel(PreTrainedModel):
    ...
    def __init__(
        ...
        elif getattr(self.config, "model_type", None) == "qwen2":
            # Qwen2 (decoder-only) shares the Llama-style structure:
            #   Qwen2ForCausalLM.model = Qwen2Model with .embed_tokens, .layers, .norm
            #   Qwen2ForCausalLM.lm_head
            # We just rebind names; bidirectional attention is enabled via the
            # forward_qwen2 monkey-patch in attention_patch.py.
            self.embed_tokens = self.model.model.embed_tokens
            self.denoise_model = self.model.model
            self.lm_head = self.model.lm_head
            del self.denoise_model.embed_tokens
