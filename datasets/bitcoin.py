...
class EllipticTimeStepDataset(Dataset):
    ...
    def _graph_tokens_to_text(self, tokens: torch.Tensor) -> str:
        ...
        <graph_sos> and <graph_eos> delimit the graph token sequence.
        <graph_reset>, <graph_ladj>, <graph_radj> are structural tokens.
        <graph_pad> is skipped.
        """
        gt = self.graph_tokenizer
        skip_ids = {gt.pad}                   # only pad is skipped now
        render_as = {
            gt.sos:   "<graph_sos>",          # was previously skipped
            gt.eos:   "<graph_eos>",          # was previously skipped
            gt.reset: "<graph_reset>",        # prefix added
            gt.ladj:  "<graph_ladj>",         # prefix added
            gt.radj:  "<graph_radj>",         # prefix added
        }
        parts: list[str] = []
